...
from .dsmil import BClassifier,IClassifier, MILNet
...
class MHIM(nn.Module):
    def __init__(self, mlp_dim=512, mask_ratio=0, n_classes=2, temp_t=1., temp_s=1., dropout=0.25, act='relu',
        ...
        if baseline == 'selfattn':
            ...
            i_classifier = nn.Sequential(
            nn.Linear(mlp_dim, n_classes))
            b_classifier = BClassifier(mlp_dim, mlp_dim, n_classes)
            self.online_encoder = MILNet(i_classifier, b_classifier)
    ...
    def forward(self, x, attn=None, teacher_cls_feat=None, i=None):
        ...
        if self.baseline == 'dsmil':
            ...
            student_logit, student_cls_feat = self.online_encoder(x, len_keep=len_keep, mask_ids=mask_ids,
                                                                  mask_enable=True)
